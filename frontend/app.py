...
from frontend.components.ui_helpers import (
    nav_bar, score_display, metric_card, signal_bar,
...
# ── Session state init ─────────────────────────────────────────────────────
for key in ["result", "chat_history", "compare_results", "last_query"]:
...
if not backend_ok:
    ...
    ⚠️  Backend offline — run: <code style="color:#FB923C">uvicorn backend.main:app --reload --port 8000</code>
...
with tab_analyze:
    ...
    <div class="search-container">
      <div class="search-label">🔍 neighborhood intelligence search</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <span style="font-family:DM Mono,monospace;font-size:10px;color:#44445A;
                 letter-spacing:1px;margin-right:8px">QUICK PICKS</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, (qn, qc) in enumerate(quick_picks):
        with qcols[i]:
            if st.button(f"{qn}", key=f"qp_{i}", help=f"{qn}, {qc}"):
                st.session_state["neighborhood_input"] = qn
                st.session_state["city_input"] = qc
                neighborhood = qn
                city = qc
    ...
    # ── Run analysis ────────────────────────────────────────────────────
    if analyze_clicked and neighborhood:
        with st.spinner(""):
            st.markdown("""
            <div style="font-family:DM Mono,monospace;font-size:12px;
                        color:#00C896;letter-spacing:1px;margin:8px 0">
            <span class="live-dot"></span> Running AI analysis pipeline...
            </div>
            """, unsafe_allow_html=True)
    ...
    if not result:
        ...
        nb_name  = raw.get("neighborhood", "")
        nb_city  = raw.get("city", "")
        ...
        <div style="display:flex;align-items:baseline;gap:14px;margin:8px 0 24px 0">
          <div style="font-family:Syne,sans-serif;font-size:32px;font-weight:800;
                      letter-spacing:-1px;color:#F0F0F8">{nb_name}</div>
          <div style="font-family:DM Mono,monospace;font-size:14px;
                      color:#8888AA">{nb_city}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with left:
            fig_gauge = make_score_gauge(score_v, tier, color)
            st.plotly_chart(fig_gauge, use_container_width=True, config={"displayModeBar": False})

            fig_bar = make_breakdown_bar(score.get("breakdown", {}))
            st.plotly_chart(fig_bar, use_container_width=True, config={"displayModeBar": False})

        # MID — Key metrics + price trend
        with mid:
            ...
            with m1:
                st.markdown(
                    metric_card(
                        f"${raw.get('median_price', 0)/1e6:.2f}M",
                        "MEDIAN PRICE",
                        f"{p1y:+.1f}% YoY",
                        p1y >= 0,
                    ), unsafe_allow_html=True
                ...
                st.markdown(
                    metric_card(f"{dom}d", "DAYS ON MARKET",
                                "fast" if dom < 12 else "moderate" if dom < 22 else "slow",
                                dom < 20),
                    unsafe_allow_html=True
                )
            with m2:
                st.markdown(
                    metric_card(
                        f"{rent:.1f}%",
                        "RENTAL YIELD",
                        "strong" if rent > 5 else "avg",
                        rent > 4.5,
                    ), unsafe_allow_html=True
                )
                st.markdown(
                    metric_card(
                        f"${raw.get('price_per_sqft', 0):,.0f}",
                        "PRICE / SQFT",
                        f"{p3y:+.1f}% (3Y)",
                        p3y >= 0,
                    ), unsafe_allow_html=True
            ...
            fig_trend = make_price_trend_chart(raw)
            st.plotly_chart(fig_trend, use_container_width=True, config={"displayModeBar": False})

        # RIGHT — Radar + signals
        with right:
            fig_radar = make_radar_chart(signals)
            st.plotly_chart(fig_radar, use_container_width=True, config={"displayModeBar": False})

            walk  = raw.get("walk_score", 0)
            trans = raw.get("transit_score", 0)
            st.markdown(signal_bar("Walk Score", walk, "#3B82F6"), unsafe_allow_html=True)
            st.markdown(signal_bar("Transit Score", trans, "#8B5CF6"), unsafe_allow_html=True)
        ...
        with sig_col1:
            st.markdown("**✅ Positive Signals**", unsafe_allow_html=False)
            st.markdown(
                chips(signals.get("positive_signals", []), "chip-positive"),
                unsafe_allow_html=True
            )
        with sig_col2:
            st.markdown("**⚠️ Risk Flags**", unsafe_allow_html=False)
            st.markdown(
                chips(signals.get("risk_flags", []), "chip-risk"),
                unsafe_allow_html=True
            )
        ...
        <span style="font-family:DM Mono,monospace;font-size:10px;color:#44445A;
                     letter-spacing:1px">SUGGESTED QUESTIONS</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        quick_qs = [
            ...
            "Compare to the Toronto average",
            "Best streets or sub-areas to target?",
        ]
        qcols2 = st.columns(4)
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            user_q = st.text_input(
                ...
                label_visibility="collapsed",
                placeholder="e.g. Is this a good market for investors right now?",
            )
...
with tab_compare:
    ...
    for i, (col, label, defaults) in enumerate(zip(
        ...
        with col:
            st.markdown(f"""<div style="font-family:DM Mono,monospace;font-size:11px;
                         color:#8888AA;letter-spacing:1px;margin-bottom:8px">{label}</div>""",
                        unsafe_allow_html=True)
            cn = st.text_input("Neighborhood", value=defaults[0], key=f"cn_{i}",
                               label_visibility="collapsed")
            cc = st.text_input("City", value=defaults[1], key=f"cc_{i}",
                               label_visibility="collapsed")
            compare_inputs.append({"neighborhood": cn, "city": cc})

    compare_clicked = st.button("Compare All →", key="compare_btn")
    ...
    if comp_data and comp_data.get("comparisons"):
        comparisons = comp_data["comparisons"]
        valid = [c for c in comparisons if "error" not in c]

        if valid:
            top_score = max(c["confidence_score"]["overall"] for c in valid)
            ...
            for i, comp in enumerate(valid):
                ...
                with score_cols[i]:
                    ...
                    <div style="background:#13131f;border:1px solid {'#00C896' if is_winner else '#1e1e30'};
                      ...
                      <div style="font-family:DM Mono,monospace;font-size:11px;color:#8888AA">{city_n}</div>
                      <div style="font-family:Syne,sans-serif;font-size:48px;font-weight:800;
                                  color:{s['tier_color']};line-height:1.1;margin:12px 0">
                        {s['overall']:.0f}
                      </div>
...
with tab_about:
    ...
    <p><strong style="color:#F0F0F8">NeighborIQ</strong> is an AI-native real estate intelligence platform
    built to demonstrate how modern AI engineering can compress hours of market research into seconds.
    It uses a multi-agent pipeline to fetch, analyze, score, and narrate neighborhood intelligence
    in real time.</p>

    <p>The system was designed to mirror how a world-class PropTech company would think about
    automating market analysis — not as a feature, but as the core product surface.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <span>The Annex · Toronto</span>
    <span>Leslieville · Toronto</span>
    <span>Liberty Village · Toronto</span>
    <span>Queen West · Toronto</span>
    <span>Scarborough · Toronto</span>
