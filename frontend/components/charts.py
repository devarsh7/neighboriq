...
CHART_LAYOUT = dict(
    ...
    xaxis=dict(gridcolor=COLORS["border"], zerolinecolor=COLORS["border"], showline=False),
    yaxis=dict(gridcolor=COLORS["border"], zerolinecolor=COLORS["border"], showline=False),
)
...
def make_score_gauge(score: float, tier: str, color: str) -> go.Figure:
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=score,
        number={"font": {"family": "Syne, sans-serif", "size": 52, "color": color}, "suffix": ""},
        gauge={
            "axis": {
                "range": [0, 100],
        ...
        title={
            "text": f"<span style='font-family:DM Mono;font-size:12px;color:{COLORS['text_muted']};letter-spacing:2px'>{tier.upper()}</span>",
            "font": {"size": 12},
        },
        domain={"x": [0, 1], "y": [0, 1]},
    ))
    fig.update_layout(**CHART_LAYOUT, height=240, margin=dict(l=20, r=20, t=40, b=10))
    return fig
...
def make_price_trend_chart(raw_data: dict) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=quarters, y=prices, fill="tozeroy",
    ...
    fig.update_layout(
        **CHART_LAYOUT, height=200, showlegend=False,
        ...
        yaxis=dict(tickformat="$,.0f", gridcolor=COLORS["border"], zerolinecolor=COLORS["border"]),
...
def make_breakdown_bar(breakdown: dict) -> go.Figure:
    ...
    fig.update_layout(
        **CHART_LAYOUT, height=220,
        ...
        xaxis=dict(range=[0, 26], gridcolor=COLORS["border"]),
